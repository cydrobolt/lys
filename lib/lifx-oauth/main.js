...
const user_settings = new ElectronSettings();
...
var LifxOauth = class {
    ...
    setUserTokenCache(token) {
        user_settings.set('lys.auth.lifx_token', token);
    }
    getTokenFromCache() {
        var user_token = user_settings.get('lys.auth.lifx_token');
        if (!user_token) {
            return false;
        }
        else {
    ...
    authorize(auth_callback) {
        // If no token is found in the cache,
        // authenticate with LIFX through OAuth
